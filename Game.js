...
class Game {
  ...
  newPlayer(client) {
    ...
    if (coords) {
      ...
      // @ekatie - Added code line 121 to 129
      this.snakes.forEach((otherSnake) => {
        if (otherSnake.client !== client) {
          try {
            otherSnake.client.write("A new player has joined the game!\n");
          } catch (e) {
            // Handle any errors if the message couldn't be sent to a specific client.
          }
        }
      });
    } else {
